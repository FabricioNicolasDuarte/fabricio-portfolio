...
const work = [
  ...
  },
  {
    ...
    title: 'Pipeline ganadero — Airflow, PySpark y Databricks',
    titleEn: 'Livestock pipeline — Airflow, PySpark and Databricks',
    summary:
      'Demo que construí para usar Apache Airflow, PySpark y Databricks: planillas de manga (pesadas, rodeos, ITH, BCS, sanidad) → Delta bronze/silver/gold → tablero con ADG, carga, ITH de riesgo y % BCS bajo. Corre en Docker (Spark local si no hay cluster). 2.000 animales × 90 días.',
    summaryEn:
      'Demo I built to use Apache Airflow, PySpark and Databricks: manga CSVs (weighings, herds, THI, BCS, health) → Delta bronze/silver/gold → dashboard with ADG, stocking, heat-stress hours and % low BCS. Runs in Docker (local Spark if there is no cluster). 2,000 animals × 90 days.',
    role: 'Diseño e implementación del pipeline y el tablero.',
    roleEn: 'Designed and implemented the pipeline and dashboard.',
    kindEn: 'Data engineering · AgTech',
    tags: ['Airflow', 'PySpark', 'Databricks', 'Delta Lake', 'Docker', 'Streamlit'],
    tagsEn: ['Airflow', 'PySpark', 'Databricks', 'Delta Lake', 'Docker', 'Streamlit'],
    cats: ['agtech', 'de', 'da', 'product'],
    href: '/agtech/lakehouse/',
    external: false,
    cta: 'Ver el proyecto →',
    ctaEn: 'See the project →',
  },
  {
    featured: true,
